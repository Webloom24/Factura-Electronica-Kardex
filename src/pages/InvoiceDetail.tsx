...
export default function InvoiceDetail() {
  ...
  if (!invoice) {
    return (
      <div className="card">
        <p>Factura no encontrada. <Link to="/invoices">← Volver</Link></p>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="form-grid">
          <div><label>Razón Social</label><p><strong>{c.company_name}</strong></p></div>
          <div><label>NIT</label><p>{c.nit}</p></div>
          <div><label>Email</label><p>{c.email}</p></div>
          <div><label>Teléfono</label><p>{c.phone}</p></div>
          <div className="form-group full-width"><label>Dirección</label><p>{c.address}</p></div>
          ...
          {c.economic_activity && <div><label>Act. Económica</label><p>{c.economic_activity}</p></div>}
        </div>
      </div>
            ...
            <tbody>
              {invoice.items.map(item => (
                <tr key={item.id}>
                  ...
                  <td>{item.sku ?? <span style={{ color: '#aaa' }}>—</span>}</td>
